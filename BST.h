...
struct BST {
		...
		// b find and print all deepest nodes (leaves)
		bool deepestNodes();
		// c BST display function
		void ascendPrintF(ostream&);
		void ascendPrintF2(BTNode*, ostream&);
		void descendPrint();
		void descendPrint2(BTNode*);
		void descendPrintF(ostream&);
		void descendPrintF2(BTNode*, ostream&);
		bool display(int, int);
		//d clone subtree to another empty tree
		bool CloneSubtree(BST t1, type);
		//e print nodes level by level
		bool printLevelNodes();
		//f print every external path of tree
		void findPath(BTNode*, Queue&);
		bool printPath();
