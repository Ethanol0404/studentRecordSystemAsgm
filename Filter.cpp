...
bool Standard_Filter() {
	...
	if (StuList.empty()) {
		...
		for (int i = 1; i <= StuList.size(); i++) {
			...
			if (cases) {
				FilterList.push_back(stu);
			}
			cases = true;
		}
		cout << "Filter done\n";
		printResult(FilterList);
		FilterList.clear();//clear all after filter
		return true;
	}
}
...
bool Range_Filter() {
	...
	cout << "Input '-' if you do want to set limit\n";
	cout << "ID From\n";
		...
		for (Student student : StuList) {
			for (int i = 0; i < student.exam_cnt; i++) {
				...
				for (int j = 0; j < student.exam[i].numOfSubjects; j++) {
					...
					subjectName=student.exam[i].sub[j].subject_code;
...
bool multiSearch() {
	...
	bool inputCheck, yearCheck, trimesterCheck, subjectMatch, duplicateCheck;
	...
	for (Student student : StuList) {
		bool match = true;
		if (!name.empty() && student.name.find(name) == string::npos) {
			match = false;
		}//case1
